...
public class ResourceKeys {
    ...
    public static final String CHUNK_LIST_DB = "RESOURCE_CHUNK_DB:[String]resource_chunk_hash:[Xml]ChunkNode";
    ...
    public static void updateDBScheme(DB db) {
        ...
        if (!db.exists(CHUNK_LIST_DB)) {
            db.hashMap(CHUNK_LIST_DB, Serializer.STRING_ASCII, Serializer.STRING).create();
        }
        ...
        if (!db.exists(RESOURCE_LINK_LOCK_DB)) {
            db.hashMap(RESOURCE_LINK_LOCK_DB, Serializer.STRING_ASCII, Serializer.STRING_ASCII).create();
        }
    }
