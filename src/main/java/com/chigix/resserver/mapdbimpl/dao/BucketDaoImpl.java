...
public class BucketDaoImpl implements BucketDao {
    ...
    public Bucket findBucketByName(String name) throws NoSuchBucket {
        ...
        Bucket b = ((Map<String, Bucket>) db.hashMap(BucketKeys.BUCKET_DB).open()).get(name);
        if (b == null) {
    ...
    public Iterator<Bucket> iteratorBucket() {
        return new BucketIterator(((ConcurrentMap<String, Bucket>) db.hashMap(BucketKeys.BUCKET_DB).open()).entrySet().iterator());
    }

    @Override
    public Bucket createBucket(String name) throws BucketAlreadyExists {
        Bucket b = new Bucket(name);
        if (((ConcurrentMap<String, String>) db.hashMap(BucketKeys.BUCKET_DB).open())
    ...
    public Bucket deleteBucketByName(String name) throws NoSuchBucket {
        Bucket b = ((ConcurrentMap<String, Bucket>) db.hashMap(BucketKeys.BUCKET_DB).open()).remove(name);
        if (b == null) {
            throw new NoSuchBucket(name);
        } else {
            db.commit();
